...
Prior to iOS 17 and the Observation framework, an ObservableObject could mark observable properties with @Published.
...
Instead, we should use the `withObservationTracking` API which, for starters, really doesn't look nice. :')
In one of my previous PaymentArchive versions, I managed to build a more-or-less stable data flow having `PaymentArchive` declared as @Observable and using `withObservationTracking` in ViewModels.
...
I realized that it is really easy to make a shareable centralized data flow using the `MainBroadcaster` utility.
This means that `PaymentArchive` requirements 1-3 were very easy to satisfy. 
And requirement #4 becomes straightforward since every ViewModel can iterate through the async sequence to get any change, then respond either synchronously or asynchronously.

In iOS 26, Apple added the `Observations` async sequence to the Observation framework, which confirmed I had been on the right track before.

## Conclusion

- Organize data flow so that `appState` is conveyed to ViewModels, the last barrier before mapping data to UI, with AsyncStreams.
